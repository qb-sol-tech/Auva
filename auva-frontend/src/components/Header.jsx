  ...
  return (
    <>
      ...
      <header className={`heading ${scrolled ? 'position-fixed' : ''}`}>
        <Container fluid>
          <div className="headershap1 position-absolute "></div>
          <div className="headershap2 position-absolute "></div>
          <div className='heading-wrap d-flex align-items-center justify-content-between position-relative'>
            {/* <div className='line position-absolute z-3'></div> */}
            <Logo />
              ...
              <ul className="d-lg-flex align-items-center">
                {navLinks.map((link, index) => (
                  <li key={index} className='d-block'>
                    <Link className="heading-link text-capitalize" to={link.to} spy={true} smooth={true} duration={600} offset={0}>{link.btn_title}</Link>
                  </li>
                ))}
              </ul>
            </nav>
            <div className="heading-actions d-flex align-items-center flex-wrap">
              <Link to='/login' className='arrowbtn'><img src={btnimg} alt="" /></Link>
